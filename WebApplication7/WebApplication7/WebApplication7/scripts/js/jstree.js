...
!(function (a) {
    ...
    if (!a.jstree) {
        var c = 0,
            ...
            i = window.document;
        (a.jstree = { version: "3.3.8", defaults: { plugins: [] }, plugins: {}, path: h && -1 !== h.indexOf("/") ? h.replace(/\/[^\/]+$/, "") : "", idregex: /[\\:&!^|()\[\]<>@*'+~#";.,=\- \/${}%?`]/g, root: "#" }),
            ...
            (a.jstree.core.prototype = {
                    ...
                    for (e in d)
                        if (
                            ...
                            ((f = e),
                                "-" !== f &&
                                "+" !== f &&
                                ((f = f.replace("--", "-MINUS").replace("+-", "-MINUS").replace("++", "-PLUS").replace("-+", "-PLUS")), (f = f.split(/-|\+/).sort().join("-").replace("MINUS", "-").replace("PLUS", "+").toLowerCase())),
                                f === c)
                        )
                            return d[e];
                    return null;
                ...
                _redraw: function () {
                    ...
                    this._model.force_full_redraw && ((c.className = this.get_container_ul()[0].className), c.setAttribute("role", "group"), this.element.empty().append(c)),
                        ...
                        ((d = this.get_node(g, !0)), d && d.length && d.children(".jstree-anchor")[0] !== i.activeElement ? d.children(".jstree-anchor").focus() : (this._data.core.focused = null)),
